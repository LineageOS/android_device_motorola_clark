...
if [ -e ../../../$OUTDIR/proprietary/vendor/lib/libtime_genoff.so ]; then
...
echo "# Prebuilt multilibs" >> $VENDOR_MAKEFILE
echo "PRODUCT_PACKAGES += \\" >> $VENDOR_MAKEFILE

LINEEND=" \\"
COUNT=`ls -1 ../../../$OUTDIR/proprietary/vendor/lib/libtime_genoff.so | wc -l`
for MULTILIB in `ls ../../../$OUTDIR/proprietary/vendor/lib/libtime_genoff.so`; do
  COUNT=`expr $COUNT - 1`
  if [ $COUNT = "0" ]; then
    LINEEND=""
  fi
    multilibname=`basename $MULTILIB`
    multimodulename=`echo $multilibname|sed -e 's/\.so$//gi'`
    (cat << EOF) >> ../../../$OUTDIR/proprietary/vendor/lib/Android.mk
include \$(CLEAR_VARS)
LOCAL_MODULE := $multimodulename
LOCAL_MODULE_OWNER := $VENDOR
LOCAL_SRC_FILES_64 := ../lib64/$multilibname
LOCAL_SRC_FILES_32 := $multilibname
LOCAL_MODULE_TAGS := optional
...
echo "    $multilibname$LINEEND" >> $VENDOR_MAKEFILE
done
echo "" >> $VENDOR_MAKEFILE
